...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Mast failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Mast failed to open a window: {e:?}. See https://zed.dev/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Mast failed to launch")
                        .body(Some(
                            format!(
...
pub fn main() {
    ...
    if args.system_specs {
        ...
        println!("Mast System Specs (from CLI):\n{}", system_specs);
        return;
    }
...
#[derive(Parser, Debug)]
#[command(name = "mast", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `mast://` scheme, or relative to the configured server URL.
    ...
    /// On macOS, the default is `~/Library/Application Support/Mast`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/mast`.
    /// On Windows, the default is `%LOCALAPPDATA%\\Mast`.
    ...
    /// Instructs Mast to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents Mast from starting, so you can't run `mast: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,
    ...
    /// Used for recording minidumps on crashes by having Mast run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run Mast in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
    ...
    /// The PID of the Mast process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    ...
    /// Unix socket path for IPC with the parent Mast process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
